...
public class JewelleryController : Controller
{
    ...
    public IActionResult Filter(string searchString)
    {
        ...
        IEnumerable<Jewellery> objJewelleryList = _unitOfWork.Jewellery.GetAll();

        if (!string.IsNullOrEmpty(searchString))
        {
            var filterResult = objJewelleryList
            ...
            if (filterResult.Any())
            {
                ...
                return PartialView("_JewellerySearch_Results", filterResult);
            }
            else
            {
                ViewBag.SearchString = searchString;
                ViewBag.NoResults = "No results found";
                return PartialView("_JewellerySearch_Results");
            }
        }

        return PartialView("_JewellerySearch_Results", objJewelleryList);
    }

    private bool ProductMatchesSearchString(Jewellery product, string searchString)
    {
    ...
    public IActionResult Index(int page = 1, int pageSize = 10)
    {
        ...
        var paginatedList = objJewelleryList.Skip((page - 1) * pageSize).Take(pageSize);

        ViewData["CurrentPage"] = page;
        ViewData["TotalPages"] = (int)Math.Ceiling(objJewelleryList.Count() / (double)pageSize);

        foreach (var jewellery in paginatedList)
        {
            var remainigQuantity = jewellery.RemainigQuantity;

            if (remainigQuantity == 0)
            {
                jewellery.StockStatus = SD.OutOfStockStatus;
            }
            else if (remainigQuantity <= 50)
            {
                jewellery.StockStatus = SD.RunningLowStockStatus;
            }
            else
            {
                jewellery.StockStatus = SD.InStockStatus;
            }
    ...
    public IActionResult Create()
    {
        var productVM = new ProductVM
        {
            ...
            // Initialize ProductCategoryList for the view model pre select the value of Jewellery
            ProductCategoryList = Enum.GetValues(typeof(ProductCategory))
                ...
                .Select(pc => new SelectListItem
                {
                    Value = pc.ToString(),
                    Text = pc.ToString(),
                    Selected = pc == ProductCategory.Jewellery
                })
        };
    ...
    public IActionResult Create(ProductVM productVM)
    {
        ...
        if (ModelState.IsValid)
        {
            if (productVM != null)
            {
                ...
                // Create an instance of Jewellery and populate base class properties
                var jewellery = new Jewellery
                {
                    ...
                    JewelleryCategory = productVM.JewelleryCategory,
                ...
                _unitOfWork.Jewellery.Add(jewellery);
                _unitOfWork.Save();

                // Save Product_Collection entries
                foreach (var productCollection in selectedCollections)
                {
                    productCollection.ProductId = jewellery.Id; // Assign the product ID
                    _unitOfWork.Product_Collection.Add(productCollection);
                }
                _unitOfWork.Save();

                TempData["success"] = "Jewellery product created successfully";
    ...
    public IActionResult Details(int? id)
    {
        ...
        if (objFromDb is Jewellery jewellery)
        {
    ...
    public IActionResult Edit(int id, ProductVM productVM)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            TempData["success"] = "Jewellery product updated successfully";
    ...
    public IActionResult Delete(int? id)
    {
        ...
        var objFromDb = _unitOfWork.Jewellery.GetFirstOrDefault(u => u.Id == id);
    ...
    public IActionResult DeletePost(int? id)
    {
        ...
        _unitOfWork.Jewellery.Remove(objFromDb);
        _unitOfWork.Save();

        TempData["success"] = "Jewellery product deleted successfully";
